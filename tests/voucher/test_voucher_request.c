...
char *create_pledge_voucher_request(struct VoucherBinaryArray *nonce, struct VoucherBinaryArray *registrar_cert) {
  ...
  char *cms = sign_pledge_voucher_request(&created_on, nonce, registrar_cert, serial_number, &cert, &key, certs);
...
static void test_sign_pledge_voucher_request(void **state) {
  ...
  registrar_cert.length = (size_t)crypto_generate_eccert(&meta, registrar_key.array, registrar_key.length, &registrar_cert.array);
...
static void test_sign_voucher_request(void **state) {
  ...
  registrar_key.length = (size_t)crypto_generate_eckey(&registrar_key.array);
  registrar_cert.length = (size_t)crypto_generate_eccert(&registrar_meta, registrar_key.array, registrar_key.length, &registrar_cert.array);
  char *pledge_voucher_request = create_pledge_voucher_request(&nonce, &registrar_cert);
  ...
  struct VoucherBinaryArray idevid_issuer = {.array = idevid_issuer_array, .length = 6};
  ...
  char *cms = sign_voucher_request(pledge_voucher_request, &created_on, serial_number, &idevid_issuer, &registrar_cert, &cert, &key, NULL, NULL);
  assert_non_null(cms);
