...
/**
 * @brief Signs a pledge voucher request using CMS for a private key (type detected automatically)
 ...
 * @param[in] proximity_registrar_cert The first certificate in the TLS server "certificate_list" sequence presented by the registrar to the pledge (array in DER format)
...
/**
 * @brief Signs a voucher request using CMS for a private key (type detected automatically)
 * and output to PEM (base64)
 *
 * Caller is responsible for freeing output PEM string
 *
 * @param[in] pledge_voucher_request The signed pledge voucher request cms structure in PEM (base64) format
 ...
 * @param[in] registrar_cert The registrar certificate (array in DER format) used to create the TLS connection with the pledge
 * @param[in] cert The certificate buffer for signing (array in DER format)
 * @param[in] key The private key buffer of the certificate (array in DER
 * format)
 * @param[in] certs The list of additional certificate buffers (DER format)
